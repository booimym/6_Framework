...
<body>
    <main>
        ...
        <section class="board-detail">  
            ...
            <c:if test = "${not empty board.imageList}">
                    ...
                    <%-- 업로드 이미지가 있는 경우 --%>
                    <c:if test = "${empty thumbnail}"> <%--섬네일x--%>
                    <c:set var = "start" value = "0"/>
                    </c:if>

                     <%-- 업로드 이미지가 있는 경우 --%>
                    <c:if test = "${not empty thumbnail}"> <%--섬네일0--%>
                    <c:set var = "start" value = "1"/>
                    </c:if>

                    <%-- imageList의 이미지 개수가 start보다 클 경우
                    썸네일 유무에 관계없이 다른 이미지가 있을 경우
                    --%>
    ...
    <%-- 방법1) JSP 내장 객체에 세팅된 값을 JS에서 얻어가는 방법1 --%>
    <%-- 화면에 숨겨놓고 js를 이용해서 값을 얻어가는 방법 --%>
    <input type = "hidden" name = "memberNo" value = "${loginMember.memberNo}">

    <%-- 방법2) JSP 내장 객체에 세팅 값을 JS에서 얻어가는 방법2 --%>
    ...
    <script>
        ...
        // *** JS에 EL/JSTL 사용 시 양쪽에 "" 또는 ''를 붙이는 것을 권장한다 ***
