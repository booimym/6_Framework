...
selectEmail.addEventListener("click",(e) => {
    ...
    //회원정보 조회할 때 아무것도 입력되지 않은 상태일 경우,
    ...
    $.ajax({
        ...
        success : (member) => {
            console.log(member); //레알 member가 나오는지 console로 찍어볼게용
            ...
            if(member == null){ // 일치 X
                ...
                //selectEamil의 다음 요소가 존재한다면 삭제하기!
                //(1)selectEmail.nextElementSibling이라고 치면 다음 요소가 선택된다!
...
function selectMemberList() {
    ...
    //tbody 이전 내용을 삭제하는 코드
    tbody.innerHTML = "";
    
    $.ajax({
        url : "/selectMemberList",
        dataType : "JSON", //응답데이터 JSON 
        success : (memberList) => {
