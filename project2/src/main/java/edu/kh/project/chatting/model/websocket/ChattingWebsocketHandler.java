...
public class ChattingWebsocketHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		// + 웹소켓에 접속한 회원의 HttpSession을 훔쳐서 가지고 있다.
	...
	// --> handlerTextMessage 수행
	// --> message.getPayLoad() == JSON데이터
	@Override
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		
		//전달 받은 내용을 확인해보겠다!
		//전달받은 내용 :{"senderNo":"3","messageContent":"하잉\n"}
		logger.info("전달받은 내용 :" + message.getPayload() );
		
		// Jackson-bind에서 제공하는 ObjectMapper 객체를 사용하겠다!
		
		// ObjectMapper : JSON을 해석해서 지정된 VO로 변환하는 객체(필드에 값 세팅)
		ObjectMapper objectMapper = new ObjectMapper();
		
		Message msg = objectMapper.readValue(message.getPayload() , Message.class);
													//JSO , 변경할 VO 클래스 (이러면 다 담김)
		
		logger.debug(msg.toString());
		
		//이걸 이제 메시지 db에 insert하겠당!!!
		int result = service.insertMessage(msg); 
		
		if(result > 0) { //삽입 성공 시
			
			//	보낸 시간이 db에는 있고 msg 객체에는 없는 상태
			// 	-> 보낸 시간 생성
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy.MM.dd hh:mm");
			msg.setSendTime( sdf.format(new Date()));
			
			// msg 객체(채팅방 번호, 대상 번호, 내용, 보낸 사람 번호, 보낸시간)
			// json으로 변환
			// 로그인한 회원 중
			// 대상번호, 보낸사람 번호가 일치하는 2명에게
			// 웹소켓으로 메시지 전달 == 실시간으로 메시지 전달...
